...
async function openaiRecommendations(userId: string, message: string, history?: {role: string, text: string}[]) {
    ...
    try {
        ...
        const systemContext = [
            ...
            "NUNCA uses respuestas genéricas. Analiza el mensaje del usuario y sus hábitos para sugerir hábitos EXTREMADAMENTE ESPECÍFICOS y ACCIONABLES.",
            "Si el usuario menciona un objetivo, recomiéndale el 'sistema' (la acción atómica mínima que lo llevará allí).",
            "",
            "Hábitos Actuales del Usuario:",
            currentHabits,
            "",
            "REGLAS OBLIGATORIAS:",
            "1. CADA HÁBITO DEBE llevar una DURACIÓN EXACTA (ej: '15 minutos', '2 páginas').",
            "2. CADA HÁBITO DEBE incluir un GATILLO (ej: 'al despertar', 'tras la cena').",
            "3. La 'reason' debe explicar la psicología detrás del comportamiento elegido.",
            "4. NO repitas los hábitos actuales.",
            "5. 'recommendedDays' debe ser un array de días en español (ej. ['Lunes', 'Miércoles']).",
            "6. 'xpReward' es un número entero entre 8 y 35.",
            "7. IMPORTANTÍSIMO: NO uses siempre 'task_alt' o 'salud'. DEBES adivinar la 'category' ('salud', 'productividad', 'mente', etc.) y DEBES usar un nombre de icono real de Material Symbols (ej: 'fitness_center', 'menu_book', 'bed', 'directions_run', 'water_drop', etc.) exacto para el hábito sugerido.",
            "",
            "DEBES RESPONDER ÚNICAMENTE CON UN JSON VÁLIDO.",
            "EJEMPLO DE ESTRUCTURA JSON (LLENA CON DATOS VARIADOS Y CONCRETOS):",
            "{",
            "  \"textResponse\": \"¡Me parece genial! Aquí tienes rutinas clave para lograrlo:\",",
            "  \"suggestions\": [",
            "    {",
            "      \"title\": \"Leer 10 páginas antes de dormir\",",
            "      \"reason\": \"Reduce la luz azul y relaja tu sistema nervioso.\",",
            "      \"frequency\": \"Diario\",",
            "      \"recommendedDays\": [\"Lunes\", \"Martes\", \"Miércoles\", \"Jueves\", \"Viernes\"],",
